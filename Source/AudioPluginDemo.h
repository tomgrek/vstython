...
return -1;
    ...
    void process(AudioBuffer<FloatType>& buffer, MidiBuffer& midiMessages, AudioBuffer<FloatType>& delayBuffer)
    {
        ...
        if (tomThread.isConnected()) {
            ...
            Pyaudio* audio = tomThread.getAudio();
            //DBG("Python at: " << audio->seqnum);
            //DBG("Im at: " << seqnum);
            if (audio->seqnum > 1){
                auto mb = audio->get();
                FloatType* memory_block = reinterpret_cast<FloatType*>(mb->getData());
                ...
                for (int ch = 0; ch < numChannels; ch++) {
                    for (auto i = 0; i < numSamples; i++) {
                        data[ch][i] = (FloatType)memory_block[i + (ch * numSamples * sizeof(float))];
                    }
                }
               ...
               //buffer.clear();
                //audio->reset();
                if (audio->seqnum != (seqnum + 1)) {
                    DBG("LOST A FRAME, WRITE SLOWER! Im at " << seqnum << " youre at " << audio->seqnum);
                }
                seqnum = audio->seqnum + 1;
           }
